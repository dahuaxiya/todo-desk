...
桌面 App 还支持飞书云盘加密备份：默认每 3 小时检查一次并保留最近 8 份，备份包含完整任务数据、设置和附件。设置页会显示当前数据大小、云端累计占用和历史版本；恢复前会自动生成本地安全快照，再完整替换数据。备份包含 API Key 等敏感设置，因此云端文件使用 AES-256-GCM 加密；请把设置页中的恢复密钥和飞书备份恢复码分开保存，新设备需要两者才能恢复。
